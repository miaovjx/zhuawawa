...
$(function() {
    ...
    ////默认分享
    h5.wxShare('湖北联通，七夕献礼啦！', '浪漫七夕沃来送礼，参与游戏赢取话费、流量，还有大牌口红等你来拿！', '湖北联通，七夕献礼啦！', h5.baseUrl + 'index.php', h5.baseUrl + 'images/jsshare.jpg');
    ...
    $('.btn_join').on('tap', function(ev) {
        ...
        $.ajax({ //判断关注与否
            url: 'index.php?mod=prize&ac=follow',
            type: 'POST',
            data: {},
            dataType: 'json',
            beforeSend: function() {
    ...
    /*$('.tk-info-wei .btn_sub').on('tap', function(ev) {
        ev.stopPropagation();
        ...
        if (username == '') {
            ...
            alert('请输入正确的手机号码!');
            return false;
        } else if (pro == '' || pro == '--请选择省份--') {
            alert('请选择省份！');
            return false;
        } else if (city == '' || city == '--请选择城市--') {
            alert('请填写城市！');
            return false;
        } else if (address == '' || address == '详细地址：') {
            alert('请填写地址！');
            return false;
        } else {
            $.ajax({
